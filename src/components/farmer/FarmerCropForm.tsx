...
export const FarmerCropForm: React.FC<FarmerCropFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upload new photo files if any
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-16">
      ...
      <div className="bg-white border-b border-emerald-100 sticky top-0 z-20 shadow-xs">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="hidden sm:inline">{lang === "hi" ? "वापस जाएँ" : "Back to Inventory"}</span>
            </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="font-extrabold text-slate-900 text-base sm:text-lg font-display">
                  {isEditing
                    ? (lang === "hi" ? "फसल विवरण व फोटो संपादित करें" : "Edit Harvest Lot & Photos")
                    : (lang === "hi" ? "नई फसल लिस्टिंग (सीधा खेत बिक्री)" : "Register New Harvest Lot")}
                </h2>
                <p className="text-xs text-slate-500 font-medium">
                  {isEditing
                    ? (lang === "hi" ? `लॉट #${editingListing.id} को अपडेट करें` : `Updating Harvest Lot #${editingListing.id}`)
                    : (lang === "hi" ? "संस्थागत खरीदारों से जुड़ने के लिए आसान 3-चरण फॉर्म" : "Direct-to-buyer farm produce listing")}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={submitting}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-5 py-2 rounded-xl text-xs shadow-sm transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
            >
              {submitting ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>{uploadingPhotos ? (lang === "hi" ? "फोटो अपलोड हो रही हैं..." : "Uploading Photos...") : (lang === "hi" ? "सहेज रहे हैं..." : "Saving...")}</span>
                </>
              ) : (
                <>
                  <Check className="w-4 h-4" />
                  <span>{isEditing ? (lang === "hi" ? "अपडेट सहेजें" : "Save Changes") : (lang === "hi" ? "मार्केटप्लेस में प्रकाशित करें" : "Publish Listing")}</span>
                </>
              )}
            </button>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-6 space-y-6">
        ...
        {errorMsg && (
          <div className="bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl p-4 text-xs font-semibold flex items-center gap-3 shadow-xs">
            <AlertCircle className="w-5 h-5 text-rose-600 shrink-0" />
            <div className="flex-1">{errorMsg}</div>
          </div>
        )}

        <form id="crop-listing-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 border border-emerald-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium mt-0.5">
                  {lang === "hi"
                    ? "पहली फोटो मुख्य फोटो (Primary Photo) के रूप में दिखाई देगी। आप क्रम बदल सकते हैं।"
                    : "The 1st photo is used as the primary lot image for buyers. Drag or reorder anytime."}
                </p>
              </div>
              ...
              {photos.length < 10 && (
                <button
                  ...
                  className="bg-emerald-50 text-emerald-800 hover:bg-emerald-100 border border-emerald-200 text-xs font-bold px-3.5 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5"
                >
                  <Upload className="w-3.5 h-3.5 text-emerald-600" />
                  <span>{lang === "hi" ? "फोटो जोड़ें" : "Add Photos"}</span>
                </button>
              )}
            </div>
            ...
            {photos.length === 0 ? (
              <div 
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-emerald-200 hover:border-emerald-400 bg-emerald-50/40 rounded-2xl p-8 text-center cursor-pointer transition space-y-2 group"
              >
                <div className="w-12 h-12 rounded-2xl bg-emerald-100 text-emerald-700 flex items-center justify-center mx-auto group-hover:scale-105 transition-transform">
                  <Upload className="w-6 h-6 text-emerald-600" />
                </div>
                <h4 className="font-bold text-slate-800 text-sm">
                  {lang === "hi" ? "फसल की फोटो अपलोड करने के लिए यहाँ क्लिक करें" : "Click or drag photos of your harvest lot here"}
                </h4>
                <p className="text-xs text-slate-500 font-medium max-w-sm mx-auto">
                  {lang === "hi" 
                    ? "खेत, फसल की बोरी या छँटाई की स्पष्ट फोटो (JPG, PNG, WEBP - अधिकतम 5MB प्रति फोटो)" 
                    : "High-resolution produce images increase buyer trust and speed up sales (JPEG, PNG, WEBP, max 5MB)"}
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={`relative group rounded-2xl overflow-hidden border bg-slate-100 aspect-square flex flex-col justify-between transition-all ${
                        isPrimary ? "ring-2 ring-emerald-500 border-emerald-400 shadow-sm" : "border-slate-200 hover:border-emerald-300"
                      }`}
                    >
                      <img
                        src={photo.url}
                      ...
                      <div className="absolute top-2 left-2 flex flex-col gap-1">
                        {isPrimary ? (
                          <span className="bg-emerald-600 text-white text-[10px] font-extrabold px-2 py-0.5 rounded-full shadow-xs flex items-center gap-1">
                            <Star className="w-3 h-3 fill-amber-300 text-amber-300" />
                            <span>{lang === "hi" ? "मुख्य फोटो" : "Primary"}</span>
                          </span>
                        ) : (
                          <span className="bg-slate-900/70 backdrop-blur text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                            #{index + 1}
                          </span>
                        )}
                      </div>
                        ...
                        <div className="flex items-center justify-between gap-1">
                          {!isPrimary && (
                            <button
                              ...
                              className="bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold px-2 py-1 rounded-md cursor-pointer transition flex-1 text-center shadow-xs"
                            >
                              {lang === "hi" ? "मुख्य बनाएं" : "Set Primary"}
                            </button>
                          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 font-bold focus:ring-2 focus:ring-emerald-500 focus:outline-hidden"
                >
                  {COMMON_CROPS.map((c) => (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-hidden"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm font-extrabold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:outline-hidden font-display"
                />
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 font-medium focus:ring-2 focus:ring-emerald-500 focus:outline-hidden"
                />
              </div>
            </div>

            {/* Organic Premium Checkbox Banner */}
            <div className="flex items-center gap-3 bg-[#F0FDF4] p-4 rounded-2xl border border-emerald-200">
              <input
                type="checkbox"
                id="organic-check"
                checked={isOrganic}
              ...
              />
              <label htmlFor="organic-check" className="text-xs font-bold text-emerald-950 cursor-pointer flex items-center gap-2">
          ...
          <div className="bg-white rounded-3xl p-6 border border-emerald-100 shadow-sm space-y-5">
            ...
            <div className="bg-[#F0FDF4] rounded-2xl p-4 border border-emerald-200 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {loadingAI && (
                  <span className="text-[11px] text-emerald-700 animate-pulse font-bold flex items-center gap-1">
                    <Loader2 className="w-3 h-3 animate-spin" />
                    <span>{lang === "hi" ? "गणना हो रही है..." : "Calculating live rates..."}</span>
                  </span>
                )}
              </div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="bg-white p-3 rounded-xl border border-emerald-100">
                      <span className="text-[10px] text-slate-400 block font-bold uppercase">{t("farmerModal.mandiBenchmark", lang)}</span>
                      <span className="font-extrabold text-slate-700 text-sm font-display">₹{aiGuidance.mandiBenchmarkPrice.toLocaleString()}</span>
                    </div>

                    <div className="bg-white p-3 rounded-xl border border-emerald-200 ring-2 ring-emerald-500 shadow-xs">
                      <span className="text-[10px] text-emerald-700 font-extrabold block uppercase">{lang === "hi" ? "एआई लक्ष्य भाव" : "AI Recommended Target"}</span>
                      <span className="font-extrabold text-emerald-900 text-base font-display">₹{aiGuidance.recommendedTargetPrice.toLocaleString()}</span>
                    </div>

                    <div className="bg-white p-3 rounded-xl border border-emerald-100">
                      <span className="text-[10px] text-slate-400 block font-bold uppercase">{t("farmerModal.recommendedRange", lang)}</span>
                      <span className="font-bold text-slate-700 text-xs sm:text-sm">₹{aiGuidance.minFairPrice.toLocaleString()} - ₹{aiGuidance.maxFairPrice.toLocaleString()}</span>
                    </div>
                  </div>

                  <p className="text-[11px] text-emerald-800 leading-relaxed font-medium bg-emerald-100/50 p-2.5 rounded-xl">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                {aiGuidance && (
                  <button
                    ...
                    className="text-xs font-extrabold text-emerald-700 hover:text-emerald-900 cursor-pointer underline underline-offset-2"
                  >
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  className="w-full pl-9 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-lg font-extrabold text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:outline-hidden font-display"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-emerald-100 shadow-sm space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">{lang === "hi" ? "जिला" : "District"} *</label>
                <input
                  type="text"
                  value={district}
                  onChange={(e) => setDistrict(e.target.value)}
                  required
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 font-semibold focus:ring-2 focus:ring-emerald-500 focus:outline-hidden"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">{lang === "hi" ? "राज्य" : "State"} *</label>
                <input
                  type="text"
                  value={state}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">{lang === "hi" ? "पिनकोड" : "Pincode"} *</label>
                <input
                  type="text"
                  value={pincode}
          ...
          <div className="flex items-center justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={onCancel}
              className="px-5 py-3 text-xs font-bold text-slate-600 hover:bg-slate-200/60 rounded-xl transition cursor-pointer"
            >
              {t("common.cancel", lang)}
            </button>
            <button
              type="submit"
              disabled={submitting}
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold px-8 py-3 rounded-xl text-xs sm:text-sm shadow-md transition-all cursor-pointer flex items-center gap-2 disabled:opacity-50"
            >
              {submitting ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                ...
                <>
                  <Check className="w-4 h-4" />
                  <span>{isEditing ? (lang === "hi" ? "फसल लॉट अपडेट करें" : "Update Harvest Lot") : (lang === "hi" ? "मार्केटप्लेस में प्रकाशित करें" : "Publish to Marketplace")}</span>
                </>
              )}
            </button>
          </div>
