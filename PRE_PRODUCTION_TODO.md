...
### 1. Add Missing Drivers to `backend/requirements.txt`
* **Problem**: The Python backend requires `psycopg2-binary` to connect to PostgreSQL and `cloudinary` to persist produce photos. Neither is currently listed in `backend/requirements.txt`.
* **Action Required**: Add the missing dependencies to `backend/requirements.txt`:
  ```text
...
| :--- | :--- | :---: | :---: |
| Add `psycopg2-binary` & `cloudinary` | `backend/requirements.txt` | 🔴 P0 | ⏳ Pending |
